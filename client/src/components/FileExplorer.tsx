...
const FileExplorer: React.FC<FileExplorerProps> = ({ yNodeMap, ydoc, provider, onFileSelect, selectedFileId }) => {
  ...
  const handleRenameSubmit = (e: React.FormEvent) => {
    ...
    if (yMapToUpdate) {
      ...
      // If the renamed file is the currently selected one, notify the parent.
      if (editingItemId === selectedFileId) {
        const fileContentId = yMapToUpdate.get("fileContentId") as string | undefined
        if (fileContentId) {
          onFileSelect(editingItemId, fileContentId, editingName)
        }
      }
    }
  ...
  const renderItem = useCallback(
    (itemId: string, depth: number) => {
      ...
      return (
        <div key={item.id} className="text-gray-300 text-xs sm:text-sm">
          <div
            style={{ paddingLeft: `${depth * 16}px` }}
            className={`flex items-center group w-full pr-1 sm:pr-2 rounded-md ${
              isSelected ? "bg-blue-800/50" : "hover:bg-gray-700/50"
            } ${isFolder ? "cursor-pointer" : ""}`}
            onClick={
              isFolder ? () => toggleFolder(item.id) : () => onFileSelect(item.id, item.fileContentId!, item.name)
            }
          >
            <div className="flex items-center flex-1 py-1 truncate">
              {isFolder ? (
                <>
                  <ChevronRight
                    className={`w-3 h-3 sm:w-4 sm:h-4 mr-1 transition-transform flex-shrink-0 ${
                ...
                <File className="w-3 h-3 sm:w-4 sm:h-4 mr-1 opacity-60 flex-shrink-0 ml-3 sm:ml-5" />
              )}
              {isEditing ? (
                <form onSubmit={handleRenameSubmit} className="flex-1">
                  <input
                    ...
                    autoFocus
                    className="bg-gray-900 text-white outline-none w-full text-xs sm:text-sm"
                  />
                </form>
              ) : (
                <span className={`truncate ${isBeingRenamed ? "opacity-50" : ""}`}>{item.name}</span>
              )}
              {isBeingRenamed && <Lock className="w-2.5 h-2.5 sm:w-3 sm:h-3 ml-1 text-yellow-500 flex-shrink-0" />}
            </div>
            {!isEditing && (
              <div className="flex opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                {isFolder && (
                  <>
                    <button
                      ...
                      className="p-0.5 sm:p-1 hover:text-white"
                      title="New File"
                    >
                ...
                <button
                  ...
                  className={`p-0.5 sm:p-1 ${isBeingRenamed ? "text-gray-600 cursor-not-allowed" : "hover:text-white"}`}
                ...
                <button
                  ...
                  className={`p-0.5 sm:p-1 ${
                    isBeingRenamed ? "text-gray-600 cursor-not-allowed" : "hover:text-red-500"
                  }`}
          ...
          {isFolder && isExpanded && (
            <div>
              {item.children.length > 0 ? (
                ...
                >
                  <span className="ml-3 sm:ml-5">(empty)</span>
    ...
    [fileSystem, selectedFileId, editingItemId, editingName, expandedFolders, renamingSet, onFileSelect],
  )
  ...
  useEffect(() => {
    ...
    const observer = () => {
      ...
      const renamingIds = new Set<string>(states.map((state: any) => state.renaming).filter(Boolean))
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-y-auto">
      ...
      <div className="flex-1 overflow-y-auto">
        {yNodeMap === null ? (
          <div className="text-gray-400 text-xs sm:text-sm">Connecting...</div>
        ) : (
